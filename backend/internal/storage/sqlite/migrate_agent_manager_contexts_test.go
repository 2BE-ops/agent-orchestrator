...
func TestAgentManagerContextMigrationPreservesInboxAndRefusesHistoryLoss(t *testing.T) {
	...
	err = goose.DownTo(db, "migrations", 173)
	gooseMu.Unlock()
	if err == nil {
		t.Fatal("downgrade erased sealed context history")
	}
	var count int
	if err := db.QueryRow(`SELECT count(*) FROM adaptive_agent_manager_contexts WHERE id='retained'`).Scan(&count); err != nil || count != 1 {
		t.Fatalf("retained context lost: %d %v", count, err)
	}
