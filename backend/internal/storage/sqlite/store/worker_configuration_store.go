...
func (s *Store) CreateConfiguredSession(ctx context.Context, rec domain.SessionRecord, snapshot domain.WorkerConfiguration) (domain.SessionRecord, error) {
	...
	err := s.inTx(ctx, "create configured session", func(q *gen.Queries) error {
		...
		return q.InsertWorkerConfiguration(ctx, gen.InsertWorkerConfigurationParams{SessionID: string(created.ID), AgentTypeID: snapshot.AgentType.ID, AgentTypeVersion: snapshot.AgentType.Version, Configuration: string(encoded), ContentHash: snapshot.ContentHash, CreatedAt: snapshot.CreatedAt})
	})
	if err != nil {
		return domain.SessionRecord{}, err
	}
