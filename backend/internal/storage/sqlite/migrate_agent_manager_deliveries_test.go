...
func TestAgentManagerDeliveryMigrationPreservesInboxAndRefusesHistoryLoss(t *testing.T) {
	...
	// SQL migration guards retain opaque history; store tests validate native
	// attribution, content hashes and the classified conversation chain.
	_, err = db.Exec(`INSERT INTO adaptive_agent_manager_contexts(id,request_id,number,controller_id,session_id,native_generation,source_owner,classification,engagement_id,previous_context_hash,snapshot,content_hash,created_at) VALUES('retained','proposal-request',1,'proposal-controller',?,'native','{}','technical','','','{}',printf('%064d',0),CURRENT_TIMESTAMP)`, string(seed.ID))
	if err != nil {
		t.Fatal(err)
	}
	_, err = db.Exec(`INSERT INTO adaptive_agent_manager_deliveries(id,request_id,context_id,controller_id,number,session_id,owner,delivery_key,state,reason,created_at,updated_at) VALUES('retained-send','proposal-request','retained','proposal-controller',1,?,'{}','adaptive-manager:proposal-request','dispatching','Retained before native effect',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`, string(seed.ID))
	if err != nil {
		t.Fatal(err)
	}
	gooseMu.Lock()
	...
	err = goose.DownTo(db, "migrations", 174)
	gooseMu.Unlock()
	if err == nil {
		t.Fatal("downgrade erased native delivery history")
	}
	var count int
	if err := db.QueryRow(`SELECT count(*) FROM adaptive_agent_manager_deliveries WHERE id='retained-send'`).Scan(&count); err != nil || count != 1 {
		t.Fatalf("retained delivery lost: %d %v", count, err)
	}
