...
func (m *Manager) createSessionWithinCap(ctx context.Context, cfg ports.SpawnConfig, project domain.ProjectRecord) (domain.SessionRecord, error) {
	...
	if err != nil {
		return domain.SessionRecord{}, wrapSpawnStageEarly(ErrSpawnCreate, err)
	}
	return rec, nil
}
...
func (m *Manager) saveAndTeardownOne(ctx context.Context, rec domain.SessionRecord, destroyRuntime bool) error {
	...
	// 5. Runtime teardown (best-effort; same pattern as Kill). Skipped when the
	// caller already destroyed the runtime (FinalizeCrashedSession reaps it up
	// front so a half-surviving tmux pane's children are cleaned before capture).
	if destroyRuntime {
		handle := runtimeHandle(rec.Metadata)
		if handle.ID != "" {
			if err := m.runtime.Destroy(ctx, handle); err != nil {
...
func (m *Manager) cleanupSessionRecord(ctx context.Context, rec domain.SessionRecord, result *CleanupResult) {
	...
	reclaim, err := m.cleanupOne(ctx, rec, ws)
	if err != nil {
...
// Returns a nil error when the workspace was reclaimed (with the reclaim status
// so an already-absent workspace can be bucketed separately); a non-nil error
// means it was left alone this run (the caller records the reason and durable
// disposition, and a later pass can retry) — most commonly because the worktree
// is dirty or a scoped shell terminal could not be confirmed closed.
