...
func (p *Plugin) authStatus(ctx context.Context) (ports.AgentAuthStatus, error) {
	...
	if status, ok := p.probeAuthStatus(ctx, report, cliOK); ok {
		return status, nil
	}
	...
	return claudeLocalAuthStatus(ctx)
}
...
func (p *Plugin) probeAuthStatus(ctx context.Context, report claudeAuthReport, cliOK bool) (ports.AgentAuthStatus, bool) {
	reported := ""
	if cliOK {
...
func claudeLocalAuthStatus(ctx context.Context) (ports.AgentAuthStatus, error) {
...
func ProviderModels(ctx context.Context, binary, workingDir string, env map[string]string) ([]ports.AgentModelInfo, error) {
	...
	if strings.TrimSpace(binary) != "" {
		if report, ok := claudeModelAuthReport(probeCtx, binary, workingDir, env); ok {
			reported = report.APIProvider
		}
	}
