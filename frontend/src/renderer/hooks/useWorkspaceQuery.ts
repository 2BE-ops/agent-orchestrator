...
const STANDALONE_WORKSPACE_NAME = "Scratchpad";
...
async function fetchWorkspaces(): Promise<WorkspaceSummary[]> {
	...
	const standalone: WorkspaceSummary = {
		...
		sessions: sessions
			...
			.map((session) => toLocalWorkspaceSession(session, STANDALONE_WORKSPACE_ID, STANDALONE_WORKSPACE_NAME)),
	};
	...
	const direct = useQuery({
		...
		queryFn: async () => {
			...
			const project = session.projectId
				...
				: ({ id: STANDALONE_WORKSPACE_ID, name: STANDALONE_WORKSPACE_NAME } satisfies Pick<WorkspaceSummary, "id" | "name">);
