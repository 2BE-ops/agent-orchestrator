...
export function ProjectSummaryPanel({ onClose, orchestrator }: { onClose: () => void; orchestrator: WorkspaceSession }) {
	...
	return (
		<aside aria-label="Project summary" className="absolute inset-0 z-overlay flex min-h-0 flex-col border-l border-border bg-background sm:relative sm:inset-auto sm:z-auto sm:w-[380px] sm:shrink-0" data-testid="project-summary-panel">
			<header className="flex h-inspector-tabs shrink-0 items-center gap-2 border-b border-border px-3">
				<div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">Project summary</p><p className="truncate text-[11px] text-passive">{orchestrator.workspaceName}</p></div>
				<Button aria-label="Refresh project summary" disabled={summary.refresh.isPending} onClick={() => summary.refresh.mutate()} size="icon" variant="ghost"><RefreshCw className={summary.refresh.isPending ? "animate-spin" : ""} /></Button>
				<Button aria-label="Close project summary" onClick={onClose} size="icon" variant="ghost"><X /></Button>
			</header>
