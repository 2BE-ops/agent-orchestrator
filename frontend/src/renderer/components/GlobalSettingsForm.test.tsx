...
vi.mock("../lib/bridge", () => ({
	aoBridge: {
		...
		telemetry: { getPolicy: getTelemetryPolicy, setEventsEnabled: setTelemetryEvents, setGithubIdentityEnabled: vi.fn(), onPolicy: onTelemetryPolicy, getBootstrap: vi.fn(), capture: vi.fn() },
	},
}));
...
beforeEach(async () => {
	...
	getTelemetryPolicy.mockResolvedValue({ eventsEnabled: false, githubIdentityEnabled: false, consentGeneration: "generation-off", updatedAt: "2026-08-28T10:15:30.000Z", acknowledged: true, consentRenewalRequired: false, state: "applied", environmentVeto: false, durabilitySupported: true });
	setTelemetryEvents.mockResolvedValue({ eventsEnabled: true, githubIdentityEnabled: false, consentGeneration: "generation-on", updatedAt: "2026-08-28T10:15:31.000Z", acknowledged: true, consentRenewalRequired: false, state: "applied", environmentVeto: false, durabilitySupported: true });
	...
	useTelemetryPolicyStore.setState({ view: { eventsEnabled: false, githubIdentityEnabled: false, consentGeneration: "generation-off", updatedAt: "2026-08-28T10:15:30.000Z", acknowledged: true, consentRenewalRequired: false, state: "applied", environmentVeto: false, durabilitySupported: true }, loaded: true, saving: false, saveError: false });
...
describe("GlobalSettingsForm", () => {
	...
	it("shows pending daemon cleanup without claiming opt-out completed", async () => {
		setTelemetryEvents.mockResolvedValue({ eventsEnabled: false, githubIdentityEnabled: false, consentGeneration: "generation-off-2", updatedAt: "2026-08-28T10:15:31.000Z", acknowledged: false, consentRenewalRequired: false, state: "cleanup_pending", environmentVeto: false, durabilitySupported: true, reason: "daemon_cleanup_pending" });
		useTelemetryPolicyStore.setState({ view: { eventsEnabled: true, githubIdentityEnabled: false, consentGeneration: "generation-on", updatedAt: "2026-08-28T10:15:30.000Z", acknowledged: true, consentRenewalRequired: false, state: "applied", environmentVeto: false, durabilitySupported: true }, loaded: true });
	...
	it("names the platform restriction instead of claiming cleanup keeps retrying", async () => {
		useTelemetryPolicyStore.setState({ view: { eventsEnabled: false, githubIdentityEnabled: false, consentGeneration: "generation-off", updatedAt: "2026-08-28T10:15:30.000Z", acknowledged: false, consentRenewalRequired: false, state: "cleanup_failed", environmentVeto: false, durabilitySupported: false, reason: "durability_unsupported" }, loaded: true });
		renderForm();
	...
	it("does not promise retries for the fail-closed view when the controller is unavailable", async () => {
		useTelemetryPolicyStore.setState({ view: { eventsEnabled: false, githubIdentityEnabled: false, consentGeneration: "unavailable", updatedAt: new Date(0).toISOString(), acknowledged: false, consentRenewalRequired: false, state: "cleanup_failed", environmentVeto: true, durabilitySupported: false, reason: "invalid_authority" }, loaded: true });
		renderForm();
		expect(await screen.findByText("Enabling is unavailable on this platform because durable consent writes are not supported.")).toBeInTheDocument();
		expect(screen.queryByText("Telemetry cleanup failed. Reporting remains disabled while cleanup retries.")).not.toBeInTheDocument();
	});
	...
	it("names the release gate when a saved opt-in cannot be honoured", async () => {
		useTelemetryPolicyStore.setState({ view: { eventsEnabled: true, githubIdentityEnabled: false, consentGeneration: "generation-on", updatedAt: "2026-08-28T10:15:30.000Z", acknowledged: true, consentRenewalRequired: false, state: "applied", environmentVeto: false, durabilitySupported: true, reason: "release_blocked" }, loaded: true });
		renderForm();
