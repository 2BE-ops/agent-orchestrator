...
| 12 | Worker result schema and typed communication | IN PROGRESS | 12a `a883468da` pushed: immutable bounded claims with ownership/context/configuration fences. 12b adds shared result service, strict submission and scoped history API/CLI, generated contracts and telemetry. Real SQLite HTTP tests, full task/CLI/telemetry/router/spec/envelope, 39 frontend tests, typecheck/build/lint PASS; full controllers retain only two recorded Windows baseline failures. Next: typed communication/durable delivery and prior findings/contracts in context. |
