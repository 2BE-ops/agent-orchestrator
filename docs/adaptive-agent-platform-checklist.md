...
| 26 | Manager routing outcomes | 18, 22 | TESTED through 18: decision-coupled task fate, bounded complete summaries and per-Type routing groups at API/CLI level; desktop views remain 22 |
| 27 | Orchestrator planning outcomes | 18, 22 | TESTED through 18: receipt-coupled task fate and bounded complete planning summaries at API/CLI level; desktop views remain 22 |
...
| 35 | Needs Human items | 20, 22 | TESTED through 20: structured reason codes with provenance, one pending request per task, task-scoped raise/resolve, project listing and the `needs-human` read phase at API/CLI level; desktop views remain 22 |
