...
| 09 | Worker snapshots and manual Agent Type launch | IN PROGRESS | 09a `6f28a74fa`, 09b `036392a6c`, 09c1 `fcd09f60e`, 09c2 `49b93fc98` pushed. Shared launch/restore, ordered resources, CLI/API, composer and launch inspector tested; 165 frontend tests and real idle native Codex Chat launch/reload pass. Migration 0152 records immutable execution history; interface and TUI/Chat harness changes activate configurations atomically with ownership. 09c3 per-turn preferences/history commit atomically after native validation and generation/sequence fencing. Full domain/registry/store/Chat, focused manager, API-spec, build, vet, typecheck and changed-code lint PASS. Next: native live controls and execution-history UI, then running-app/restart/heterogeneous validation. Baseline Windows full-suite gaps remain recorded. |
