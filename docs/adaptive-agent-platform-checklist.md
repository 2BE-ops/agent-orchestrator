...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Classified conversation boundary pushed as `b13f173df`. 14e2 atomically seals input and reserves bounded sends, retains uncertain conversation ownership, escalates exhausted no-send attempts, and attributes proposals to received/cumulative classified context. Both native modes use the existing shared guard/relay; exact-byte, forged/stale/busy/unknown/changed-policy tests PASS. Full storage/domain/service/spec suites, build/typecheck/generation/lint PASS; full native/HTTP retain recorded Windows baselines. Next: daemon inbox consumption, native CLI protocol and public delivery/context history, then stage-15 semantic selection. No live Manager claim yet. |
