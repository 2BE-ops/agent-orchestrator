...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Start service pushed as `e64a34d2b`, classification foundation as `09e9f72a8`. 14d2 wires daemon-native start plus strict API/CLI start/current/exact-controller reads. Body authority/bounds, retry after disabled policy, scope, role and error-envelope tests PASS. Frontend preserves Manager identity and excludes it from worker/orchestrator populations; all 146 affected frontend tests and typecheck PASS. API generation/build/new-diff lint and full service/CLI/telemetry/skill/router/spec suites PASS. Full HTTP/daemon retain only recorded Windows baselines. Next: classified, durable inbox delivery and native protocol, then stage-15 semantic selection/integration. No live Manager claim yet. |
