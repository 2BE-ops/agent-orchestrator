...
| 13 | Evaluator and attributable performance history | IN PROGRESS | Reviewer policy pushed as `2d67c10f7`. 13c3b adds migration 0167 and atomic bounded context/run/audit provenance, exact result/config/PR guards, immutable scope and one-time native launch witness; ordinary reviews keep empty scope. Three context-store tests and full domain/ports/SQLite/task/review-service/spec suites, sqlc/API generation, typecheck/build/lint PASS. Full review engine retains the known Node-shim PATH baseline. Next: native context consumption and fenced submission, evaluator review attribution, derived completion and usage/performance denominators. |
