...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Governance API/CLI pushed as `25f7417ea`. 14b1 adds durable per-project controller admission, atomic exact-Type/Skills session binding, native-generation intent/resolution and release/restore fences (0170). Eight-writer admission/seed races, restart, audit rollback, policy rechecks, native uncertainty, restore-vs-release and downgrade tests PASS. Full domain/ports/SQLite/store/Manager/registry suites, build, sqlc and affected pinned lint PASS (0 issues). Next: existing native lifecycle integration, inbox and structured tools; production Manager launch remains unwired. |
