...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Governance storage pushed as `d479165ce`. 14a3 adds the shared service, strict project API, bounded history/audit, five thin CLI commands, generated contracts and telemetry redaction. New service/HTTP/CLI tests, full CLI/service/router/spec/telemetry/skill-assets, build, typecheck, 40 frontend tests and affected pinned lint PASS (0 issues). Full HTTP retains two known Windows failures; daemon retains its known CWD cleanup failure. Next: durable native admission, inbox and controller tools; no production manager launch yet. |
