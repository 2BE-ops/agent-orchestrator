...
| 12 | Worker result schema and typed communication | IN PROGRESS | 12a implements immutable schema-v1 results (0162), bounded claims/corrections, idempotency/version fences, attempt/native ownership and exact context/effective configuration attribution. Two domain and five store tests, full domain/SQLite/store/CDC/context/task, build and affected-package lint PASS. Next: shared result service/API/CLI, typed communication/durable delivery and prior findings/contracts in context. Stage 11c3 `0cee71119` pushed. |
