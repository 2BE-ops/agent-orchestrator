...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Proposal persistence (0172) pushed as `e36eeba74`. 14c4 adds generation-fenced native proposal service/API/CLI and scoped history. Real SQLite/HTTP tests cover exact raw receipts/retries, malformed output, stale sources, authority/body bounds, no worker launch and history after termination; service/CLI/telemetry tests PASS. Contract review fixed null/missing candidate arrays. API generation/build/typecheck/40 API-client tests and pinned affected lint PASS (0 issues); full HTTP retains only two recorded Windows baselines, all other affected suites PASS. Next: production controller start and sealed delivery context, using the requested 15.0 classification foundation before content delivery; no live Manager claim yet. |
