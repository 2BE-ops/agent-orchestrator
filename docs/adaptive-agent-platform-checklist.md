...
| 15 | Automatic selection/composition/dynamic creation and classification integration | IN PROGRESS | TESTED 15a: request-scoped candidate API/CLI/native protocol v2 checks exact-version clearance, Type/Skill selection permissions, required capabilities and shared native readiness; bounded pages retain exclusions and continuation. Lattice/activation/ownership/native failures, HTTP scope, CLI envelopes and protocol hash tests PASS; build/typecheck/generated API/new-diff lint PASS. Persistent semantic decisions, composition/evolution/approval bounds, registry/task/knowledge selectors, per-item inspector/public delegation history, live message/review context gates remain. Upward findings/results stay legal. |
...
| 9 | Automatic Agent Manager selection | 15 | IN PROGRESS: TESTED candidate compatibility/tools (15a); persisted semantic decision/application pending |
...
| Ownership | Manager cannot spoof human origin or select/modify/version prohibited types | IN PROGRESS: TESTED 15a candidate selection refuses protected Types and Skills independently of modify/version permission. Existing registry writes enforce independent policy; native decision/application and creation-tool integration pending. |
