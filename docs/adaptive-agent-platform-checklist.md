...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Native proposal API/CLI pushed as `3f37733fd`; classification foundation pushed as `09e9f72a8`. 14d1 adds dedicated start/current/history service over shared native engines. Eight concurrent retries authorize one native call; unseeded/uncertain admission is retained, with no implicit relaunch. TUI/Chat service-to-engine tests and authority/bounds/failure tests PASS; full Manager service, build and affected new-diff lint PASS. Full session-manager has only eight recorded Windows baselines. Next: daemon/API/CLI wiring with correct frontend Manager role, then classified inbox delivery/semantic selection. No live Manager claim yet. |
