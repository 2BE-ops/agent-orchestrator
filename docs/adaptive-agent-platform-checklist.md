...
| 10 | Task DAG, revisions, criteria and leases | IN PROGRESS | 10a pushed `1adac4552`, 10b pushed `2ce6564e8`; 10c shared task service, 10 HTTP routes, generated contracts and HTTP-only `ao task` tested. Scope/history/criteria/Type validation/pagination/errors, full CLI/router/spec/domain, 38 frontend tests, typecheck/build and changed-code lint PASS; only known full-controller Windows failures. Lease/atomic-dispatch persistence tested in 10b. Next: native launch/restore integration; shared scheduler admission remains stage 16. |
