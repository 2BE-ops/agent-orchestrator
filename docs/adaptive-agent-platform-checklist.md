...
| 09 | Worker snapshots and manual Agent Type launch | IN PROGRESS | 09a–09c3 pushed through `7ba445a7f`. Shared launch/restore, resources, CLI/API/composer, atomic interface/harness/per-turn configurations tested. 09c4 adds bounded history API and live inspector; 75 frontend tests, domain/session/API-spec suites, build/vet/typecheck/changed-code lint PASS. Real native Codex reply-only turn, Low effort change/history, lazy details, renderer reload and full desktop/daemon restart with live-host reconnection PASS. Original launch remains unchanged. Next: provider-owned live controls and heterogeneous validation. Baseline Windows controller failures and empty never-prompted native thread resume gap recorded in validation. |
