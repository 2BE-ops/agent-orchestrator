...
| 13 | Evaluator and attributable performance history | IN PROGRESS | Review evaluation pushed as `d39133058`. 13d1 derives current completion from latest attempt/result/assessment and current SCM/review facts, exposes proof IDs/reason in task reads, and retains native leases. New revision/result/attempt, cancellation, newer review and changed checks/heads invalidate current completion while history survives. Full domain/ports/storage/task/CLI/spec, generation/build/typecheck/lint PASS; final store/task 14.619s/1.172s. Next: usage/performance evidence and denominators. Scheduler completion/dependency admission remains 16; native review restore 23 and live provider evidence 25; Windows gaps retained. |
