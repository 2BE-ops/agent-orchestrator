...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Native start API/CLI pushed as `eed13c893`. 14e1 adds migration 0174 and classified, versioned exact-input receipts. Pinned clearance survives live Type edits; the conversation chain retains cumulative sensitivity and engagement across requests and TUI/Chat replacement generations. Nine lattice combinations, cross-engagement refusals, upward input, ownership fences, concurrency/restart/bounds/SQL/rollback/migration tests PASS. Full domain/ports/SQLite/store/Manager service, build/sqlc and pinned affected lint PASS. Next: durable transport, native protocol and proposal-to-input attribution, then stage-15 semantic selection/integration. No live Manager claim yet. |
