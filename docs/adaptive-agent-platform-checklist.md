...
| 09 | Worker snapshots and manual Agent Type launch | IN PROGRESS | 09a `6f28a74fa`, 09b `036392a6c`, 09c1 `fcd09f60e` pushed. Shared launch/restore, ordered resources, CLI/API, composer and launch inspector tested; 165 frontend tests and real idle native Codex Chat launch/reload pass. Migration 0152 records immutable execution history; interface change/rollback and TUI/Chat harness transfer now activate configurations atomically with ownership. Full registry/SQLite store suites, focused manager integration, backend build and changed-code lint PASS. Next: live model settings and execution-history UI, then complete stage-09 running-app/restart/heterogeneous validation. Baseline Windows full-suite failures remain recorded. |
