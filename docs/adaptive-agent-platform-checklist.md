...
| 10 | Task DAG, revisions, criteria and leases | IN PROGRESS | 10a task/criteria persistence (0154–0155) tested: immutable history, graph/CAS/authority, concurrency, atomic audit/CDC rollback, reopen and upgrade/downgrade. Full domain/SQLite/store/CDC, backend build, sqlc and pinned changed-package lint PASS. Next: exclusive leases, atomic attempt-to-session seed association and task service/API. Stage 09 pushed as `7b04f5477`. |
