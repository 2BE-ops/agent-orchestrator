...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Native TUI/Chat admission and pinned restore pushed as `9d14d8af6`. 14c1 adds durable exact-task/criteria/policy inbox references and terminal receipts (0171), queue/history bounds, atomic audit/CDC and restart-safe exact retries. Competing submissions, scope/authority, cancellation, policy/revision drift, rollback, immutable history and migration tests PASS. Full domain/ports/SQLite/store/Manager/registry suites, build, sqlc and affected pinned lint PASS (0 issues). Next: inbox API/CLI, native structured proposals and production start; no live Manager claim yet. |
