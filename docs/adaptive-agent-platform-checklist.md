...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Native TUI/Chat integration pushed as `9d14d8af6`; durable inbox (0171) pushed as `54b298285`. 14c2 exposes strict project-scoped inbox/history/terminal receipts through service, six HTTP operations and thin CLI commands. Authority denial, exact retries, cross-project isolation, disabled-policy fences, pagination/body bounds, null pending state and no launch/task-intent effects PASS. API generation/build/typecheck/40 API-client tests and pinned affected lint PASS (0 issues). Full HTTP retains only two recorded Windows baselines; remaining affected suites PASS. Next: generation-fenced native structured proposals and production start; no live Manager claim yet. |
