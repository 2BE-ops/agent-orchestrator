...
| 11 | Knowledge, bounded context and manifests | IN PROGRESS | 11a `3c2abd08b` pushed. 11b adds shared knowledge service, six daemon routes, HTTP-only CLI authoring/search/history and generated contracts. Focused HTTP/CLI/service tests, full CLI/service/telemetry/router/spec/envelope, 39 frontend tests, typecheck/build/changed-code lint PASS. Full controllers have only two recorded Windows baseline failures. Next: bounded context manifests and native launch integration; management UI follows stage 22. |
