...
| 13 | Evaluator and attributable performance history | IN PROGRESS | CI collector/provenance (0165–0166) pushed as `4dd972735`. 13b adds scoped evaluation service/API/CLI and strict caller-verdict exclusion. Real-SQLite HTTP, service/CLI boundaries, full task/CLI/router/spec/telemetry/skillassets, 39 frontend tests, typecheck/build/API generation and pinned lint PASS. Full controllers retain two recorded Windows failures. Next: independent command/artifact/review evidence, derived completion and performance aggregation with denominators. |
