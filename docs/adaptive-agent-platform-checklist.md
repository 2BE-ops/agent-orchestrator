...
| 13 | Evaluator and attributable performance history | IN PROGRESS | Native review engine pushed as `3501a9d23`. 13c3d adds generation-fenced, bounded native review results with atomic audit, exact historical retry and generic-path bypass prevention; HTTP/CLI preserve per-result generation. Full domain/ports/SQLite/store/review-service/task/CLI/spec, focused HTTP, generation, build, typecheck and pinned lint PASS; final store rerun 13.546s. Next: shared review request/inspection service/API/CLI and evaluator attribution; then derived completion and usage/performance denominators. Native task-review restore reconciliation remains 23; Windows suite gaps retained. |
