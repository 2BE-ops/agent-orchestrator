...
| 10 | Task DAG, revisions, criteria and leases | IN PROGRESS | 10a pushed `1adac4552`; 10b exclusive leases/attempts/atomic configured-session association (0156) tested. Frozen pins, heartbeat/activity, expiry reservation, exact owner recovery, stale fencing, concurrent/idempotent dispatch and fault rollback PASS; full domain/SQLite/store/CDC, build, sqlc and pinned lint PASS. Next: native launch/restore integration and task service/API; scheduler admission remains stage 16. |
