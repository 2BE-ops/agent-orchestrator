...
| 12 | Worker result schema and typed communication | IN PROGRESS | Result protocol `5d70d8dda` and message/journal persistence `e14b916e2` pushed. 12c2a adds shared message service, strict submit and project/task timeline API/CLI, generated contracts and public delivery provenance. Real SQLite HTTP, identity/CLI tests, full task/CLI/telemetry/domain/store/router/spec/envelope, 39 frontend tests, typecheck/build/lint PASS. Full controllers retain only the two recorded Windows gaps. Next: native delivery/reconciliation and prior findings/contracts in context. |
