...
| 13 | Evaluator and attributable performance history | IN PROGRESS | Current completion pushed as `69d7297e4`. 13d2a adds one-row-per-attempt evidence, pinned configuration and mixed-change flags, historical assessment/first-pass/CI/review facts, cohort bounds and session-native usage with null/zero/estimate/overflow distinctions. Full affected domain/ports/SQLite/store/task, sqlc/build/lint PASS; final store/task 13.029s/0.897s. Next: aggregates/API/CLI in 13d2b. Scheduler completion/dependency admission remains 16; native review restore 23, desktop performance 22 and live provider evidence 25; Windows gaps retained. |
...
| 25 | Type/Skill/version performance | 13, 22 | IN PROGRESS: attempt evidence/usage and retained version attribution tested in 13d2a; aggregates/API/CLI and desktop views pending |
