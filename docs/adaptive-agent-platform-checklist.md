...
| 12 | Worker result schema and typed communication | IN PROGRESS | Result protocol `5d70d8dda` pushed. 12c1 adds bounded typed messages, scoped threads/results, shared ownership checks and exclusive delivery journal (0163), with immutable provenance and conservative crash outcomes. One domain/eight store message tests plus result/migration regression, full domain/SQLite/store/CDC/task/context, backend build and affected lint PASS. Next: service/API/CLI, native delivery/reconciliation and prior findings/contracts in context. |
