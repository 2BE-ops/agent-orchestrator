...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Inbox API/CLI pushed as `40c41c3b5`. 14c3 retains exact native proposals/parser outcomes (0172), checks current and confirmed native generations, task/policy pins, and bounded correction. Empty/malformed exhaustion or explicit escalation atomically records Needs Human; parsed selections remain unapplied for stage 15. TUI/Chat attribution, competing retries, restart, stale/uncertain owners, mutable intent, audit rollback and migration/history tests PASS. Full domain/ports/SQLite/store/Manager/registry suites, build, sqlc and pinned affected lint PASS (0 issues). Next: native proposal API/CLI, production controller start and sealed delivery context; no live Manager claim yet. |
