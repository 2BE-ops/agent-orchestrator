...
| 13 | Evaluator and attributable performance history | IN PROGRESS | Provenance pushed as `1d016571d`. 13c3c reuses the review engine/native launchers with frozen Type/Skills/criteria, per-pass immutable files and native identity, exact scope retries, ownership exclusion and uncertain-launch retention. Focused trigger/restore/task-review (0.933s), worker snapshot (1.380s), review-service/OpenCode/ports, build and pinned lint PASS. Full reviewer/manager Windows gaps recorded. Next: shared service/API/CLI, fenced reviewer submission and evaluator attribution; then derived completion and usage/performance denominators. Native task-review restore reconciliation remains 23. |
