...
| 13 | Evaluator and attributable performance history | IN PROGRESS | 13a adds immutable CI evaluation collection/attribution (0165) and per-check snapshot provenance (0166), frozen named checks, exact commits, idempotency and bounds. Domain/store/migration tests, full affected persistence/SCM/session/task/HTTP/spec/CLI, typecheck/build/generation/lint PASS. Next: service/API/CLI, independent command/artifact/review evidence, derived completion and performance aggregation with denominators. |
