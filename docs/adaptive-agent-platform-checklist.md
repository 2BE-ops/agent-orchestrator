...
| 13 | Evaluator and attributable performance history | IN PROGRESS | Review request/inspection pushed as `6c354bcf3`. 13c3f adds exact-result native review attribution and deterministic qualitative criterion decisions; generic approvals, missing launch witnesses, stale heads, corrected results and newer incomplete passes cannot yield acceptance. Domain violation matrix and real SQLite lifecycle/reopen tests PASS; full domain/storage/services/spec, focused HTTP, generation/build and pinned lint PASS. Next: derived task completion and usage/performance denominators. Native task-review restore remains 23; live reviewer evidence 25; Windows baseline gaps retained. |
...
| 15.0 | Context classification foundations | NOT STARTED | User amendment: one migration plus domain/store; ordered technical < engagement < mission on tasks and ProjectKnowledge, versioned Agent Type max_context_class clearance; engagement scope, pinned clearance, classified sealed manifests and versioned inspectable delegation artifacts. Deterministic violation tests required. |
| 15 | Automatic selection/composition/dynamic creation and classification integration | NOT STARTED | Candidate rationale, permissions, approval gates, creation bounds; compatibility excludes task classes above Type clearance; Context Builder gates every payload, manifest and knowledge selection against the attempt's pinned Type version. Registry/task/knowledge selectors and per-item classifications in the session inspector. Upward findings/results remain legal. |
...
| Context classification | Deterministically reject embedding technical < engagement < mission material above the attempt's pinned Agent Type clearance in delegation payloads, sealed manifests and knowledge selections; live Type edits cannot raise an existing attempt's clearance. Violation tests mirror Ownership. | NOT STARTED: stages 15.0/15 |
| Engagement isolation and upward flow | Per-client/per-target engagement context is shared only within its engagement; technical findings/results can flow into higher-class context. Test cross-engagement refusal and legal upward flow independently of clearance. | NOT STARTED: stages 15.0/15 |
| Classified provenance and authoring | Versioned inspectable delegation artifacts and sealed per-item classifications survive restart; Manager compatibility rejects tasks above clearance; registry, task composer and knowledge authoring expose selectors and session inspector displays retained classifications. | NOT STARTED: stages 15.0/15/25 |
...
| Evidence integrity | Pending/unknown checks, stale head, malformed results, self-reported tests, evaluator retries | TESTED: immutable evidence/retries, CI/artifact/mergeability and native review violation tests through 13c3f; derived completion still pending |
