...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Stage 13 pushed as `b410f1200`. 14a1 introduces distinct project-owned manager session identity (0168), separate conversation scope, active-controller uniqueness, immutable role/project and downgrade history protection; generic spawn is fenced. Focused migration/store/API/service and full affected domain/ports/SQLite/store/session/spec suites PASS. Full HTTP has two known Windows failures; full session lint has two unchanged Windows path-conversion findings. Next: durable manager policy/inbox, admission and native tools; no production manager launch yet. |
