...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | 14a1 identity pushed as `ceaf040e4`. 14a2 adds immutable exact-Type governance, bounded opt-in evolution policy, human-only CAS changes and atomic audit/CDC (0169). Migration downgrade/retention, eight-writer races, audit rollback, policy escalation, disabled-Type history and restart tests PASS. Full domain/ports/SQLite/store/CDC, build, sqlc and pinned affected lint PASS (0 issues). Next: governance API/CLI, durable inbox, admission and native tools; no production manager launch yet. |
