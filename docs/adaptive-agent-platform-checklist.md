...
| 14 | Persistent Agent Manager service/controller/tools | IN PROGRESS | Governance API/CLI pushed as `25f7417ea`; durable admission/native fences (0170) pushed as `b27949f99`. 14b2 wires dedicated admission, single-use native generations and exact pinned Type/Skills restore through existing TUI/Chat engines. Native launch/replay/reopened restore, invalid authority and uncertain-launch retention tests PASS. Build and affected lint PASS (0 issues); full session-manager suite retains only eight recorded Windows baselines; ports/chat/session services PASS. Found and fixed an extra storage read affecting ordinary restore. Next: durable inbox, structured tools and production start; no live Manager claim yet. |
