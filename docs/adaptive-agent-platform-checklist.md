...
| 15 | Automatic selection/composition/dynamic creation and classification integration | IN PROGRESS | TESTED 15a: request-scoped candidate API/CLI/native protocol v2 checks exact-version clearance, Type/Skill selection permissions, capabilities and shared native readiness. TESTED 15b: migration 0176 immutable attributed decisions, transactional policy/content/configuration rechecks, routing-only selection proof, bounded shared parser/semantic corrections and Needs Human. Race/forgery/rollback/restart/migration tests PASS; full domain/ports/SQLite/store/registry/Manager/spec suites, build/typecheck/API/sqlc/new-diff lint PASS (recorded Windows HTTP/native baselines retained). Automatic assessment/recovery and public decisions, composition/evolution/approval bounds, selectors, classified inspector/public delegation history and live message/review gates remain. |
...
| 9 | Automatic Agent Manager selection | 15 | IN PROGRESS: TESTED compatibility/tools (15a) and immutable transactional decisions/shared correction budget (15b); automatic assessment/recovery and scheduler consumption pending |
...
| 13 | Manager inspects available Types/Skills | 14-15 | IN PROGRESS: 14e3 seals native read-only Type/Skill CLI commands; candidate compatibility/selection evidence and live consumption remain 15/25 |
| 14 | Persistent Agent Manager service/controller/tools | TESTED | Durable native claims pushed as `c93ba87d7`. 14e3 wires the post-reconciliation inbox consumer, single native admission, fair durable scans, conservative recovery and generation-specific versioned CLI protocol. Exact input/delivery API/CLI history and telemetry redaction are exposed. Real SQLite consumer, native proposal race, boundary/fairness/unknown-start tests PASS; full storage/domain/service/CLI/spec/telemetry/skills, 40 frontend tests, build/typecheck/API/lint PASS. Full HTTP/daemon retain recorded Windows baselines. Semantic selection/evolution remains 15, uncertain controls/recovery 20/23 and live demonstration 25. |
...
| Ownership | Manager cannot spoof human origin or select/modify/version prohibited types | IN PROGRESS: TESTED 15a compatibility and 15b decision transaction refuse protected Types/Skills and policy races; generic callers cannot forge selected resolution. Existing registry writes enforce independent policy. Automatic native assessment and creation-tool integration pending. |
| Context classification | Deterministically reject embedding technical < engagement < mission material above the attempt's pinned Agent Type clearance in delegation payloads, sealed manifests and knowledge selections; live Type edits cannot raise an existing attempt's clearance. Violation tests mirror Ownership. | TESTED foundation 15.0: exact-version authority, live activation, forged labels, required-task denial, SQL filtering before limit and atomic classified delegation seal. Manager sealed-input clearance also TESTED in 14e1; native receipt verification and proposal attribution TESTED in 14e2; automatic consumer boundary TESTED in 14e3; selection and live-message/review integration pending. |
