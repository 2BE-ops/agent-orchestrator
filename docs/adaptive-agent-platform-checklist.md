...
| 13 | Evaluator and attributable performance history | IN PROGRESS | Evaluation API/CLI pushed as `a91722303`. 13c1 collects bounded PR/review/lifecycle facts, preserves historical hashes and verifies exact-commit mergeability. Domain/store/HTTP tests, full persistence/task/review-service/SCM/router/spec, generation/typecheck/build/pinned lint PASS. Unchanged review launcher has one Windows Node PATH-order failure, recorded for 24. Next: reviewer Type/criteria provenance, independent local verification, derived completion and usage/performance denominators. |
