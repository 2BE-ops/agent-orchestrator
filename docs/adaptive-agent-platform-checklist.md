...
| 13 | Evaluator and attributable performance history | IN PROGRESS | CI build/test/lint pushed as `ee5772d91`. 13c3a pins exact reviewer Type/version and optional different-Type/harness policy in frozen criteria; planning transaction rejects invalid references, worker edits denied, historical policy survives registry changes/restart. Domain/SQLite/task/context/spec/HTTP tests, API generation, typecheck, build and pinned lint PASS. Next: native reviewer configuration/criteria provenance and consumption, derived completion and usage/performance denominators. |
