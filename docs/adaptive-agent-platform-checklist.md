...
| 12 | Worker result schema and typed communication | IN PROGRESS | Native delivery/recovery `9fc3568cb` pushed. 12d adds bounded prior findings, frozen dependency result selection and incoming contracts to sealed context with exact provenance. Three SQLite context tests plus full domain/persistence/task/context/message suites, backend build, sqlc and affected-package lint PASS. Next: native submission instructions. Human resolution of uncertain recipients integrates in 20; live provider consumption remains 25. |
