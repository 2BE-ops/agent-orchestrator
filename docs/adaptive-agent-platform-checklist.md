...
| 11 | Knowledge, bounded context and manifests | IN PROGRESS | 11a adds versioned knowledge persistence (0159–0160): candidate/reviewed/invalidated/superseded/deleted history, pins, exact sources, task relevance, scoped literal search and worker ownership checks. Focused domain/store/migration tests and full domain/SQLite/CDC/build/affected-package lint PASS. Next: shared knowledge service/API, bounded context manifests and native launch integration. |
